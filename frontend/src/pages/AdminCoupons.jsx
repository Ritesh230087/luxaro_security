          ...
          <tbody>
            {coupons.map(c => (
              <tr key={c._id}>
                ...
                <td>
                  <span className={`status-dot ${c.isActive ? 'active' : 'inactive'}`}></span>
                  {c.isActive ? 'Active' : 'Inactive'}
                </td>
                <td>
                  <div className="btn-group">
                    <button className="btn-text edit" onClick={() => handleEdit(c)}>Edit</button>
                    <button className="btn-text delete" onClick={() => handleDeleteClick(c._id)}>Delete</button>
                  </div>
                </td>
              </tr>
            ))}
                  ...
                  {selectedLogs && selectedLogs.length > 0 ? (
                    selectedLogs.map((log, i) => (
                      <tr key={i}>
                        ...
                        <td>{log.appliedProducts?.map(p => p.name).join(', ') || 'N/A'}</td>
                        <td>{log.usedAt ? new Date(log.usedAt).toLocaleString() : 'N/A'}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td colSpan="4" className="empty-state">No usage history available</td>
                    </tr>
                  )}
                </tbody>
              </table>
      ...
      {showDeleteModal && (
        <div className="modal-backdrop" onClick={() => setShowDeleteModal(false)}>
          <div className="modal-box delete-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-footer">
              <button className="btn-text cancel" onClick={() => setShowDeleteModal(false)}>
                Cancel
              </button>
              <button className="btn-text delete-confirm" onClick={confirmDelete}>
                Delete
              </button>
            </div>
          </div>
