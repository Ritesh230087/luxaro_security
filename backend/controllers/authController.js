...
const LOCKOUT_DURATION = 1 * 60 * 1000; // 30 minutes
...
const checkRateLimit = (identifier, maxAttempts = 10, windowMs = 1 * 60 * 1000) => {
  const now = Date.now();
...
exports.register = async (req, res) => {
  try {
    ...
    if (!checkRateLimit(`register_${clientIp}`, 10, 1 * 60 * 1000)) {
  ...
  try {
    ...
    if (user.isLocked && user.isLocked()) {
      ...
      return res.status(423).json({ 
        success: false, 
        message: `Account locked due to multiple failed attempts. Try again in 15 minute(s).` 
      });
    }
...
// ==========================================
// LOGOUT - MONGODB SESSION CLEANUP
// ==========================================
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    try {
      ...
      if (!checkRateLimit(`forgot_${clientIp}`, 10, 1 * 60 * 1000)) {
        return res.status(429).json({ 
          success: false, 
...
exports.verifyOTP = async (req, res) => {
  try {
    ...
    // Rate limiting
    if (!checkRateLimit(`otp_${cleanEmail}`, 10, 1 * 60 * 1000)) {
      return res.status(429).json({ 
        success: false, 
