...
exports.updateProduct = async (req, res) => {
  try {
    ...
    // ✅ FIXED: Handle Images properly - only update if new ones uploaded
    if (req.files && req.files.length > 0) {
      const newImages = req.files.map(file => `/uploads/products/${file.filename}`);
      // Replace old images with new ones (or append - choose your logic)
      updateData.images = newImages; // Replace mode
    ...
    // Parse and validate comparePrice
    if (updateData.comparePrice !== undefined) {
      const cp = parseFloat(updateData.comparePrice);
      if (!isNaN(cp)) {
        if (cp > 0 && cp < updateData.price) {
